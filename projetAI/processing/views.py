...
def Process_func(request):
  ...
  def state(monf):
    if monf==0 :
        return("Broke")
    elif monf>0 and monf<=1 :
        return("Unstable")
    elif monf >1 and monf <=1.5:
        return("Stable")
    else :
        return("Rich")
  ...
  if(response) != [] :
    for l in list(response):
      ...
      print("Unable to determine financial state.")
